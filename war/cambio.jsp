...
<script type="text/javascript">
	...
	function checkRadio(){
		if((radioSelectedTr!=null) && (radioSelectedTr!="00000000")){
			getVal('trE'+radioSelectedTr).setAttribute("class", "textoRadio");
			getVal('trS'+radioSelectedTr).setAttribute("class", "textoRadio");
			getVal('trN'+radioSelectedTr).style.display = "none";
			getVal("divActual"+radioSelectedTr).style.display = "block";
			$('#trE'+ radioSelectedTr + ',' + '#trS'+radioSelectedTr +",.hrsStatus"+radioSelectedTr).addClass('hrsSeleccionados').css({"color": "#0080C0"});
		}		
	}
...
<body class="" style="background: #FFFFFF" onLoad="checkRadio();inicial();">
   ...
   <form name="frmCambio" action="/generarsolicitud" method="post" target="_self"> 
      ...
      <tbody>
         ...
         for (BeanHorario eHorario : e) {
          ...
          if (!id.equals("00000000")) {
           ...
           if (enabled == 0) {
            ...
            $('#trE'+ <%=id%> + ',#trS'+<%=id%>+',.hrsStatus'+<%=id%>).css({"color": "#0080C0", "font-weight": "bold", "background-color":"rgb(235, 235, 228)"});});
            </script>
            <%
           }
       ...
       <p id="tr<%=iReng%>" style="background-color: #F0F9FE">
         ...
         <td class="hrsStatus<%=id%>" style="font-family: 'Arial'; font-size: 13px;">
