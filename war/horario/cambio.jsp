...
<%	
	String sHorarioAct = (String)(request.getAttribute("HorarioActual")!=null?request.getAttribute("HorarioActual"):"00000000");
	...
	BeanMensaje message =  (BeanMensaje)(request.getAttribute("message")!=null?request.getAttribute("message"):new Entity("Mensaje"));
...
<script type="text/javascript">
	...
	function checkRadio(){
		if((radioSelectedTr!=null) && (radioSelectedTr!="00000000")){
			getVal('trE'+radioSelectedTr).setAttribute("class", "tblSeguimientoD1");
			getVal('trS'+radioSelectedTr).setAttribute("class", "tblSeguimientoD1");
			getVal('trN'+radioSelectedTr).setAttribute("class", "tblSeguimientoD1");
			getVal("divActual"+radioSelectedTr).style.display = "block";
		}		
	}
	...
	function habilitaEnviar(){
		...
		if(radioChecked && checkBuChecked){
	...
	$(document).ready(function(){
		...
		$("#b1").click(function(){
			frmCambio.action="/buscasupervisores";
...
<body class="BodyPrincipal" style="background: #FFFFFF" onLoad="checkRadio();inicial();">
  <div id="ContenedorPrincipal">
    <form name="frmCambio" action="/generarsolicitud" method="post" target="_self">
      ...
      <table width="100%" cellspacing="0" cellpadding="0" border="0" class="contenedor">
        <tr>
          <td width="55%;">
            <table width="83%" cellspacing="1" cellpadding="1" class="tblSeguimientoT" border="0">
              <tr>
                <td>Modificaci&oacute;n de Horario Laboral</td>
              </tr>
            </table>
          </td>
        </tr>
          ...
          <td>
            <fieldset class="fieldset-2">
              <legend class="tituloVentanaIzq">Horarios disponibles</legend>
                <table width="100%" cellpadding="2" cellspacing="2" border="0" class="tblCandidato">
                  <thead>
                    <tr>
   ...
   if(e!=null){
	   for(BeanHorario eHorario:e) {
			...
			if(!id.equals("00000000")){
				...
				if(enabled!=0 && !id.equals(sHorarioAct)){
...
%>
                    <tr id="tr<%=iReng%>">
					  <td id="trE<%=id%>" class="<%=sClaseTr%>" style="text-align: center;"><%=entrada%></td>
					  <td id="trS<%=id%>" class="<%=sClaseTr%>" style="text-align: center;"><%=salida%></td>
					  <td id="trN<%=id%>" class="<%=sClaseTr%>" align="center">
					    <input type="radio" onClick="onclickRadio();" name="rbdgp" id="<%=iReng%>" value="<%=id%>" style="display: <%=block2%>;" />
					    <div id="divActual<%=id%>" style="display: <%=block1%>">Actual</div>
					    <div id="divSaturado<%=id%>" style="display: <%=block%>">Horario Saturado</div>
					  </td>
					</tr>
...
%>
                    <tr id="tr1">
                      <td colspan="4" class="tblSeguimientoI1">No se cuenta con informaci&oacute;n</td>
                    </tr>
        <%}%>
                </tbody>
              </table>
		...
		<tr>
		  <td colspan="4">
			<table width="100%" cellspacing="10" cellpadding="5" border="0">
			  <tr>
				<td width="55%;" class="tblSeguimientoI1">He validado previamente el cambio con mi superior</td>
				<td width="10%;" class="tblSeguimientoI1">
				  <input type="checkbox" name="validate" id="validate" onClick="habilitaEnviar();"/>
				</td>
				<td width="15%;">&nbsp;</td>
				<td width="20%;">&nbsp;</td>
		      </tr>
			  ...
			  <tr>
				...
				<td colspan="3" class="tblSeguimientoI1">
					<input type="text" class="cajaTextoEx" size="30" id="nom_autorizador"
				    name="nom_autorizador" onKeyUp="habilitaEnviar();" 
				    value="<%=request.getAttribute("nomAutorizador")%>" disabled>
				    <img src="../config/images/BTPGLUPA.gif" style="width: 40px; height: 20px; vertical-align: top;"
				         onMouseOver="javascript:handCursor(this);" onMouseOut="pinterCursor(this);" id="b1" />
				</td>
			  </tr>
			  <tr>
				<td colspan="4">
