...
{	
	...
	public List<BeanUsuarioDirectorio> BuscarUsuario(){
		...
		try { 
			...
			{
				ArrayList<BeanCyge> arrBeanCygeClean = new ArrayList<BeanCyge>();
			    boolean flag= false;
			    
			    for (BeanCyge registroCyge: listaRegistrosCyge)
			    {
			     if (arrBeanCygeClean.size() == 0){
			      arrBeanCygeClean.add(registroCyge);
			     } else {
			      for (int i=0;i < arrBeanCygeClean.size();i++) {
			                if(registroCyge.getEmail().equals(arrBeanCygeClean.get(i).getEmail())){
			                 flag = true;
			                 i = arrBeanCygeClean.size() + 1;
			                }
			            }
			      if (!flag){
			       arrBeanCygeClean.add(registroCyge);
			    ...
			    for (BeanCyge registroCyge: arrBeanCygeClean)
			    {
